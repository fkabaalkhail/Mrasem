...
export default function ActivitiesPage() {
  ...
  useEffect(() => {
    ...
    void sb
      ...
      .order("id")
      .then(({ data, error }) => {
        if (!error && data) setRows(data as Activity[]);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-mrasem-preiwinki">{t("Activities", "الأنشطة")}</h1>
          <p className="text-sm text-gray-600">{t("Manage activity listings", "إدارة قوائم الأنشطة")}</p>
        </div>
        <Link
          href="/activities/new"
          className="inline-flex rounded-lg bg-mrasem-awaki px-4 py-2 text-sm font-medium text-white hover:opacity-90"
        >
          {t("Add activity", "إضافة نشاط")}
        </Link>
      </div>
      <div className="mt-6">
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        {loading ? (
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {filtered.map((r) => {
                const previewSrc = resolveImageSrc(r.image_name, "activities");
                return (
                <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/activities/${r.id}`} className="text-mrasem-awaki hover:underline">
                      {t("Edit", "تعديل")}
                    </Link>
                  </td>
                </tr>
