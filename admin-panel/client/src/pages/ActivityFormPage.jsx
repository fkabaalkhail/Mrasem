...
export default function ActivityFormPage() {
  ...
  const [form, setForm] = useState({
    name: '', rating: 0, category: '', location: '', description: '', city: '',
  });
  ...
  useEffect(() => {
    if (!isEdit) return;
    apiGet(`/api/activities/${id}`)
      .then((data) => {
        setForm({
          name: data.name || '', rating: data.rating || 0,
          category: data.category || '', location: data.location || '',
          description: data.description || '', city: data.city || '',
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) await apiPut(`/api/activities/${id}`, fd);
      else await apiPost('/api/activities', fd);
      setToast({ message: `Activity ${isEdit ? 'updated' : 'created'} successfully`, type: 'success' });
      setTimeout(() => navigate('/activities'), 500);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate('/activities')} className="text-[#213C2E] hover:underline text-sm">&larr; Back</button>
        <h1 className="text-2xl font-bold text-[#31231B]">{isEdit ? 'Edit Activity' : 'Add Activity'}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 max-w-3xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[#31231B] mb-1">Category *</label>
            <input name="category" value={form.category} onChange={handleChange} className={inputClass('category')} />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-[#31231B] mb-1">Location</label>
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button type="button" onClick={() => navigate('/activities')} className="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300">
            Cancel
          </button>
        </div>
      </form>
