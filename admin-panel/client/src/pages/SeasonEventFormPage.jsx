...
export default function SeasonEventFormPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const isEdit = Boolean(id);

  const [form, setForm] = useState({
    name: '', category: '', location: '', description: '', city: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) await apiPut(`/api/season-events/${id}`, fd);
      else await apiPost('/api/season-events', fd);
      setToast({ message: `Season event ${isEdit ? 'updated' : 'created'} successfully`, type: 'success' });
      setTimeout(() => navigate('/season-events'), 500);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate('/season-events')} className="text-[#213C2E] hover:underline text-sm">&larr; Back</button>
        <h1 className="text-2xl font-bold text-[#31231B]">{isEdit ? 'Edit Season Event' : 'Add Season Event'}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 max-w-3xl">
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button type="button" onClick={() => navigate('/season-events')} className="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300">
            Cancel
          </button>
        </div>
      </form>
